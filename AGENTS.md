...
1. Implementation in `packages/core/src/providers/<name>.ts` — implements `ImageProvider` interface
2. Pi extension in `packages/pi-image-gen-<name>/.pi/extensions/image-gen-<name>/index.ts` — thin wrapper with provider-specific tool schema

Provider-specific params (model, style, quality, etc.) use `Type.Union` of `Type.Literal` values.
